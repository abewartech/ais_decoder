    ...
    def on_message(self, message):
        ...
        try:          
            ...
            if not ((self.xmin <= float(proc_msg.get('decoded_msg',{}).get('x',self.xmin)) <= self.xmax) and \
                ...
                # producer = self.connection.ensure(self.sink, self.sink.publish, errback=self.errback, interval_start = 1.0)
                self.sink.publish(proc_msg, routing_key=proc_msg['routing_key']) 
                # self.producer.publish(proc_msg, routing_key=proc_msg['routing_key'])
                # with producers[self.connection].acquire(block=True) as producer:
                #     producer.publish(
                #         proc_msg, 
                #         routing_key=proc_msg['routing_key'])
